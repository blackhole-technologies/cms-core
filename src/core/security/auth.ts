...
function sign(data: string): string {
  return createHmac('sha256', sessionSecret!).update(data).digest('base64url'); // URL-safe base64
}
...
export function destroySession(res: ServerResponse, req?: IncomingMessage): void {
  ...
  if (req) {
    ...
    if (sessionData?.sessionId) {
      ...
      if (dbPool) {
        dbPool
          .query(`DELETE FROM sessions WHERE session_id = $1`, [sessionData.sessionId])
          .catch((err: unknown) => {
            const message = err instanceof Error ? err.message : String(err);
...
export function invalidateSessions(userId: string): number {
  ...
  if (count > 0) {
    if (dbPool) {
      dbPool.query(`DELETE FROM sessions WHERE user_id = $1`, [userId]).catch((err: unknown) => {
        const message = err instanceof Error ? err.message : String(err);
