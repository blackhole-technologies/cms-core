...
export function generateToken(sessionId: string): string {
  ...
  const signature = createHmac('sha256', csrfSecret).update(data).digest('base64url');
...
export function validateToken(sessionId: string, token: string): boolean {
  ...
  const expectedSignature = createHmac('sha256', csrfSecret).update(data).digest('base64url');
...
export function createMiddleware(
  ...
  const { exemptPaths = ['/api/'], exemptMethods = ['GET', 'HEAD', 'OPTIONS'] } = options;
 ...
 */
function getTokenFromRequest(req: IncomingMessage, ctx: CSRFRequestContext): string | null {
