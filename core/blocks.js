 ...
 * Structure: { blockTypeId: { variantName: VariantConfig, ... }, ... }
 */
const blockVariants = {};
 ...
 * @param {string} variantName - Variant identifier (e.g., 'compact', 'highlighted')
 * @param {object} config - Variant configuration
 * @param {string} config.label - Human-readable variant label
 * @param {string} [config.description] - Variant description
 * @param {string} [config.preview_image] - Preview image URL
 * @param {object} [config.schema] - Variant-specific schema (extends base schema)
 * @param {object} [config.defaults] - Variant-specific defaults (extends base defaults)
 * @param {string} [config.template] - Variant-specific template
 * @param {function} [config.render] - Variant-specific render function
 *
 * WHY VARIANTS:
 * Different contexts need different block appearances:
 * - 'compact' search for sidebars, 'full' search for dedicated pages
 * - 'highlighted' call-to-action blocks vs regular blocks
 * - Different styling/layout without creating duplicate block types
 */
export function registerBlockVariant(blockType, variantName, config) {
  ...
  if (!config || typeof config !== 'object') {
    throw new Error('Variant config must be an object');
  }

  if (!config.label) {
    throw new Error('Variant must have a label');
  }

  // Check that block type exists
  if (!blockTypes[blockType]) {
    throw new Error(`Block type "${blockType}" does not exist. Register the block type first.`);
  }

  // Initialize variant registry for this block type
  if (!blockVariants[blockType]) {
    blockVariants[blockType] = {};
  }

  // Store variant config
  blockVariants[blockType][variantName] = {
    name: variantName,
    label: config.label,
  ...
  // Fire hook
  if (hooksService) {
    hooksService.fire('block:variantRegister', { blockType, variantName, config });
  }
}
 ...
 *
 * @param {string} blockType - Block type ID
 * @returns {object} - Object mapping variant names to configs
 */
export function getBlockVariants(blockType) {
  return blockVariants[blockType] || {};
}
 ...
 * @returns {object|null} - Variant config or null
 */
export function getBlockVariant(blockType, variantName) {
  return blockVariants[blockType]?.[variantName] || null;
}
 ...
 *
 * @param {string} blockType - Block type ID
 * @returns {array} - Array of variant configs
 */
export function listBlockVariants(blockType) {
 ...
 */
export function hasBlockVariant(blockType, variantName) {
  return !!(blockVariants[blockType] && blockVariants[blockType][variantName]);
}

/**
 * Build variant config by merging base block type with variant overrides
 *
 * @param {string} blockType - Block type ID
 * @param {string} variantName - Variant name
 ...
 *
 * WHY INHERITANCE:
 * Variants extend the base block, not replace it. A 'compact' variant
 * inherits the base schema but may add/override specific fields.
 */
export function buildVariantConfig(blockType, variantName) {
  const baseType = blockTypes[blockType];

  if (!baseType) {
    throw new Error(`Block type "${blockType}" not found`);
  }

  // If no variant specified or variant doesn't exist, return base
  if (!variantName || !blockVariants[blockType]?.[variantName]) {
    return { ...baseType };
  }

  const variant = blockVariants[blockType][variantName];

  // Merge base and variant configs
  return {
    ...baseType,
    ...
    // Merge schemas (variant extends base)
    schema: {
      ...baseType.schema,
      ...variant.schema,
    },
    // Merge defaults (variant extends base)
    defaults: {
      ...baseType.defaults,
      ...variant.defaults,
    },
    // Variant can override template and render
    template: variant.template || baseType.template,
    render: variant.render || baseType.render,
  };
}
